...
from services.integrations import sync_to_hubspot, notify_slack, trigger_n8n
...
def _serialize(doc: dict) -> dict:
    """Recursively convert datetime -> iso string for Mongo insertion."""
...
@api.get("/")
async def root():
    return {"service": "AI SDR Agent", "status": "ok", "time": _now_iso()}
...
async def get_settings(current=Depends(get_current_user)):
    return await _get_settings(current["id"])
...
async def update_settings(payload: IntegrationSettings, current=Depends(get_current_user)):
    ...
    await db.settings.update_one(
        {"owner_id": current["id"]}, {"$set": data}, upsert=True
    )
    return data


# ---------- Leads ----------
async def _run_ai_pipeline(lead: Lead, settings: dict) -> Lead:
    """Qualify lead + generate email + fire integrations. Mutates & persists."""
    try:
    ...
    # Fire integrations
    qual_dict = lead.qualification.model_dump()
    lead_dict = lead.model_dump()
    if settings.get("auto_sync_hubspot", True):
        r = await sync_to_hubspot(lead_dict, qual_dict, settings)
        lead.activities.append(Activity(type="hubspot_sync", message=r["message"], metadata=r))
    if settings.get("auto_notify_slack", True):
        r = await notify_slack(lead_dict, qual_dict, settings)
        lead.activities.append(Activity(type="slack_notified", message=r["message"], metadata=r))
    if settings.get("auto_trigger_n8n", False):
        r = await trigger_n8n(lead_dict, qual_dict, settings)
        lead.activities.append(Activity(type="n8n_triggered", message=r["message"], metadata=r))

    lead.updated_at = datetime.now(timezone.utc)
    doc = _serialize(lead.model_dump())
    await db.leads.replace_one({"id": lead.id}, doc, upsert=True)
    return lead


@api.post("/leads", response_model=Lead)
async def create_lead(payload: LeadCreate, current=Depends(get_current_user)):
    ...
    lead = await _run_ai_pipeline(lead, settings)
    return lead


@api.post("/leads/public", response_model=Lead)
async def create_lead_public(payload: LeadCreate, owner_email: str = Query(...)):
    """Public lead capture form endpoint. Attributes lead to given account owner."""
    ...
    await db.leads.insert_one(_serialize(lead.model_dump()))
    settings = await _get_settings(owner["id"])
    lead = await _run_ai_pipeline(lead, settings)
    return lead
...
async def list_leads(
    ...
    docs = await db.leads.find(query, {"_id": 0}).sort("created_at", -1).to_list(limit)
    return docs
...
async def update_status(lead_id: str, payload: LeadStatusUpdate, current=Depends(get_current_user)):
    doc = await db.leads.find_one({"id": lead_id, "owner_id": current["id"]}, {"_id": 0})
    if not doc:
        raise HTTPException(status_code=404, detail="Lead not found")
    lead = Lead(**doc)
    ...
    lead.activities.append(Activity(
        type="status_change", message=f"Status changed from {old} to {payload.status}"
    ))
    lead.updated_at = datetime.now(timezone.utc)
...
async def regen_email(lead_id: str, current=Depends(get_current_user)):
    ...
    lead.activities.append(Activity(
        type="email_generated", message="AI regenerated personalized email"
    ))
    lead.updated_at = datetime.now(timezone.utc)
    await db.leads.replace_one({"id": lead.id}, _serialize(lead.model_dump()))
    return lead
...
async def analytics_summary(current=Depends(get_current_user)):
    ...
    for l in leads:
        ...
        if isinstance(c, str):
            ...
            day = c.date().isoformat() if c else None
        if day:
            daily[day] += 1
    timeline = sorted([{"date": k, "count": v} for k, v in daily.items()], key=lambda x: x["date"])[-14:]

    # AI insights (top 3 recommendations from recent leads)
    recent = sorted(leads, key=lambda x: x.get("created_at", ""), reverse=True)[:5]
    insights = []
    for l in recent:
        q = l.get("qualification") or {}
        if q.get("qualification_summary"):
            insights.append({
                "lead_id": l["id"],
                "company": l["company"],
                "name": l["name"],
                "score": q.get("score"),
                "summary": q.get("qualification_summary"),
